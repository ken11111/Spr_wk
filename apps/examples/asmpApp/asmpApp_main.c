...
/* Worker ELF path */
#define WORKER_FILE "/mnt/spif/myasmp"
...
int main(int argc, char *argv[])
{
  ...
  ret = mpmutex_init(&mutex, MYASMPKEY_MUTEX);
  if (ret < 0)
    {
  ...
  ret = mpmq_init(&mq, MYASMPKEY_MQ, mptask_getcpuid(&mptask));
  if (ret < 0)
    {
  ...
  ret = mpshm_init(&shm, MYASMPKEY_SHM, 1024);
  if (ret < 0)
    {
  ...
  ret = mpmq_send(&mq, MSG_ID_MYASMP, (uint32_t) &data);
  if (ret < 0)
    {
