...
**バージョン**: 1.11 (要求トレーサビリティ駆動の追加対策 M-38 (時間分割) / M-39 (OSD) / M-40 (再生, 保留) + H-18 + SC-17)
...
>   - **M-40** (Q11 アプリ内再生) — 家庭用スコープでは「外部プレーヤー前提」を継続、本書では保留候補として記録のみ
> - 新規 Hazard **H-18 (証拠性不足)** + 新規 SC **SC-17 (証拠性 + ファイル管理)**
> - Phase 12.1 工数: 11 → 13 人日 (M-38 追加), Phase 12.2: 20-23 → 23-28 人日 (M-39 追加)
> - 実データ評価 (`figures/ds_13_requirement_achievement.png`):
>   - **Q16 Must <1s 達成率 99.98%** (構造的天井 134ms に対し要求 1s で十分余裕)
>   - **Q17 ドロップ率 ≤30% 目標 vs 実測 62.3%** (Phase C) → M-1+M-24+M-34+M-36 で改善見込みだが目標達成は未確証
>   - **Q19 自動再接続** ADR-002 v1.1 の数値 (6.74 → 2.77) は Phase C 実データ平均 5.57 で部分整合
>
...
| **H-18** | 映像に時間情報・場所情報が記録されておらず、録画の証拠性が低い状態 (v1.11 追加 — Q14 要求の未実装由来) | L-3 (証拠性) |
...
| **SC-17** | 録画ファイルが法的・運用上の証拠として使えるよう、時間情報を映像に焼き込み + 時間分割で 1 ファイルを管理しやすいサイズに維持 (v1.11) | H-18 |
...
|---|---|---|---|---|---|---|---|
| **M-38** | MP4 ファイル時間分割 (30 分 or 1 時間ごと自動 close → 次ファイル open) — 法的監査・運用上の利便性 | **Q9 ファイル分割**, UCA-C1.2 | **2** | PC | 12.1 | TC で 30 分単位の分割 + index 連続性確認 | M-3 (ローテーション) と一体実装可 |
| **M-39** | 映像 OSD 重畳 (タイムスタンプ + デバイス ID + Phase など) を JPEG decode 直後に書き込み — 証拠性向上 | **Q14 OSD**, **H-18**, L-3 (完全性) | **3-5** | PC (受信時重畳) or Spresense (encode 前) | 12.2 | TC で OSD 表示確認 + ファイル再生時の可読性 | Spresense 側は **M-36 Sub-Core オフロード候補** |
| **M-40** | アプリ内動画再生機能 (egui に再生 UI 追加) — viewer ファイル一覧 + 再生ボタン | **Q11 再生機能** | **5-10** | PC | **判断必要** | ユーザビリティテストで一定操作完遂率 | **家庭用スコープでは「外部プレーヤー前提」も妥当** |

#### 6.5g.1 M-39 の実装場所判断

M-39 (OSD) の実装場所は **2 つの候補**:

| 実装場所 | メリット | デメリット |
|---|---|---|
| **(A) PC viewer 側** (decode 後の RGBA に焼き込み) | Rust の image クレートで容易、Spresense 負荷ゼロ | 録画前のフレームに OSD なし → 「証拠性」要件と乖離 |
| **(B) Spresense 側** (JPEG encode 前 or 後に書込み) | 録画ファイルに直接 OSD 焼き込み = 証拠性確保 | **CPU 負荷 +5-10% 推定** = M-36 Sub-Core オフロード候補 |

→ **証拠性要件 (H-18, L-3)** を満たすには (B) Spresense 側実装が望ましい。**M-36 Sub-Core で実装すればメインコア負荷 +0%**。

#### 6.5g.2 M-40 のスコープ判断

`FUNCTIONAL_REQUIREMENTS.md` Q11 では「アプリ内再生機能 = 未実装、外部プレーヤー前提」と既に整理されているが、本書 v1.11 では:

- ✅ **採用**: 運用ランブック (M-6) で外部プレーヤー利用手順を文書化
- ⚠ **保留** (M-40 候補): アプリ内再生が「あればより良い」程度なら Phase 13+ で再評価

→ **本書では M-40 は「保留候補」として記録のみ**。実装提案はせず、外部プレーヤー前提を継続 (Q11 と整合)。
...
| **M-38, M-39** (v1.11) | **SC-17 (証拠性 + ファイル管理)**, M-39 は L-3 (完全性侵害) にも対応 |
...
|---|---|---|---|---|
| **12.1** (即着手) | 1-2 週間 | M-3, M-5, M-6, M-17, M-20, M-25, M-27, M-33, **M-38 (時間分割, v1.11)** | **13 人日** | UCA-C1.2, O.1, O.2, B1.4, PHYS.1, VIEW.2, MD.1〜4, O.5, **Q9 (時間分割)** |
| **12.2** (主要対策 + Observability + OSD) | 4-6 週間 | M-7, M-18〜M-22, M-31, M-35a, M-35b, **M-39 (OSD 重畳, v1.11)** | **23-28 人日** | UCA-D1.1, FBM.*, STREAM.2, VIEW.2, B1.5, B3.1, PWR.2, 下流ボトルネック特定可能化, **+ Q14 (OSD) / H-18 (証拠性)** |
