...
> **⚠ 未実装機能注記 (2026-05-03)**: 本書には複数の機能が記述されているが、要求書 v1.0 §9 「⚪ 未達成」で確定された通り、以下は **未実装**:
> - Q8 自動ローテーション (1GB 上限到達時の古いファイル削除) — X-5a
> - Q9 時間分割 (1 時間ごとなど) — X-5b、現在は**イベント分割のみ実装**
> - Q11 アプリ内再生 — X-5c
> - Q14 OSD 重畳 (映像へのタイムスタンプ書き込み) — X-5d
>
> 詳細は [`../quality/FUNCTIONAL_SPEC_AUDIT.md`](../quality/FUNCTIONAL_SPEC_AUDIT.md) §4, [`../use_cases/primary_use_cases.md`](../use_cases/primary_use_cases.md) UC-3, UC-4 参照。
