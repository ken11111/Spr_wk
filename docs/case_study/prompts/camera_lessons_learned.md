...
カメラアプリケーション開発の全体フローは以下の図で確認できます：
- [camera_application_flow.puml](../../diagrams/camera_application_flow.puml) - カメラアプリ開発フロー

関連する図：
- [config_dependencies.puml](../../diagrams/config_dependencies.puml) - カメラに必要なCONFIG依存関係
- [build_flow.puml](../../diagrams/build_flow.puml) - ビルド手順
...
## 📊 Phase 3.0 追加学習事項 (2025-12-31)

### 発見 11: WSL2 OpenGL 制限と Windows クロスコンパイル

**問題**:
- WSL2 環境で GUI アプリ（eframe/egui）が OpenGL エラーで動作しない
- WSLg でも性能が不安定（ソフトウェアレンダリング）

**解決策**:
MinGW-w64 による Windows クロスコンパイル:
```bash
# WSL2 で Windows .exe をビルド
sudo apt install mingw-w64
rustup target add x86_64-pc-windows-gnu
cargo build --target x86_64-pc-windows-gnu --features gui
```

**学習**:
- WSL2 の X11/OpenGL は本番環境に不向き
- クロスコンパイルで Windows ネイティブビルドが可能
- `.cargo/config.toml` で linker 設定必須

---

### 発見 12: PC 側パイプライン最適化（Option A）

**問題**:
- Spresense 側: 30 fps 送信
- PC 側: 16-20 fps 受信（40% 損失）
- GUI スレッドで JPEG デコード (8-10ms) + テクスチャ (2-3ms) = 10-13ms/frame

**原因**:
GUI スレッドが JPEG デコードとレンダリングの両方を担当し、60 FPS サイクルに間に合わない

**解決策 (Option A)**:
```rust
// キャプチャスレッド:
let img = image::load_from_memory(&jpeg_data)?;  // 8-10ms
let rgba = img.to_rgba8();
tx.send(DecodedFrame { width, height, pixels })?;

// GUI スレッド:
texture.set(color_image);  // 2-3ms のみ
```

**性能改善**:
- Before: 16-20 fps (GUI スレッド負荷 10-13ms)
- After: **25-30 fps** (GUI スレッド負荷 2-3ms)

**学習**:
- I/O と CPU 処理を並列化すべき
- GUI スレッドは軽量に保つ（60 FPS レンダリング維持）
- デコード済み RGBA データを渡すことで GUI 負荷削減
...
**問題**:
- 初期テストで Decode 時間が 0ms と表示（測定バグ）
- どこがボトルネックか不明
...
**GUI 表示**:
```
📊 FPS: 25.3 | ⏱ Decode: 8.7ms | 📡 Serial: 18.2ms | 🖼 Texture: 2.1ms
```

**学習**:
- 推測せず測定する（"Premature optimization is the root of all evil"）
- 各処理の時間を分離して測定
- ユーザーに可視化することで問題特定が容易

---

### 発見 14: Spresense タイムアウト vs 連続送信

**問題**:
- Phase 1.5 では 3 秒タイムアウトで終了
- Phase 3.0 では長時間テストが必要
...
while (g_running) {
    usleep(100000);  /* 100ms */
    // Ctrl+C で g_running = false
}
```

**学習**:
- テスト用タイムアウトと本番用連続動作を切り替え可能に
- シグナルハンドラ（SIGINT/SIGTERM）で graceful shutdown
- USB エラーやキューフル時の自動停止も実装
...
**問題**:
- コード変更後にビルドしても、古いコードが実行される
- NuttX ビルドシステムが変更を検出しない
...
**学習**:
- 重要な変更後は必ず `make clean`
- フラッシュ後にシリアルログで確認（コード変更が反映されているか）
- ビルドタイムスタンプだけでなく実行ログで検証
...
- Camera thread + USB thread = 3.76倍改善

**PC 側 (Phase 3.0)**:
- Capture thread (decode) + GUI thread (texture) = 1.56倍改善

**学習**:
システム全体の性能は最弱リンクで決まる。両側を最適化して初めて真の性能が出る。

### 教訓 7: クロスコンパイルで開発環境の制約を突破
...
**解決**:
- MinGW-w64 クロスコンパイル
- Windows ネイティブ実行で真の性能測定

---

**作成者**: Claude Code (Sonnet 4.5)
**作成日**: 2025-12-16
**最終更新**: 2025-12-31 (Phase 3.0)
**プロジェクト**: Security Camera Phase 1A → Phase 3.0
