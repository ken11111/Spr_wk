...
> **⚠ 数値根拠注記 (2026-05-03)**: 本書記述の「テストカバレッジ 92%」は **未計測 / 未検証** の数値。RTM v5.0 §A 監査結果と QUALITY_REQUIREMENTS.md §7.5 で ⚪ 未計測 と判定済。「Phase 5 品質向上」は文書整備の Phase であり、実テスト整備ではない。詳細は [`../quality/FUNCTIONAL_SPEC_AUDIT.md`](../quality/FUNCTIONAL_SPEC_AUDIT.md) §7 参照。Phase 12 でテストカバレッジ実測 + ベースライン確立タスク (X-8) を検討。
