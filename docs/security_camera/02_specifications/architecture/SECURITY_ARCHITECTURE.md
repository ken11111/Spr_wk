...
> **⚠ 重要 (2026-05-01 追記)**: 本書は **セキュリティ設計提案** であり、現行の実装は本書記述の機構 (TLS 1.3 / mTLS / JWT 認証 / 多層防御 / 監査ログ等) を**ほぼ実装していない**。設計と実装の乖離は [`../quality/risk_analysis/SECURITY_GAP_ANALYSIS.md`](../quality/risk_analysis/SECURITY_GAP_ANALYSIS.md) で透明に開示している。本書を読む際は、記述内容が「実装済」ではなく「設計提案」である点に注意。Phase 12 以降の実装判断は SECURITY_GAP_ANALYSIS.md 参照。
