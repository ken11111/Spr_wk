...
**バージョン**: 2.0 (Phase 7.3.3対応)
**日付**: 2026-01-15
**目的**: Spresense から PC への MJPEG フレーム転送

## パケットタイプ一覧

| Sync Word | パケットタイプ | サイズ | 用途 | Phase |
|-----------|---------------|--------|------|-------|
| `0xCAFEBABE` | MJPEG Frame | 14 + N bytes | 単一JPEGフレーム | 1.0 |
| `0xCAFEBEEF` | Metrics | 50 bytes (固定) | Spresense統計 | 4.1 |
| `0xCAFEBABF` | Batch Frame | 可変 | 複数フレームバッチ | 7.2a |

**エンディアン**: すべてLittle Endian
...
**総サイズ**: 12 bytes (header) + N bytes (JPEG data) + 2 bytes (CRC) = **14 + N bytes**

**サイズ計算式**: `packet_size = 12 + jpeg_size + 2`
...
## Phase 4.1: Metricsパケット仕様

**バージョン**: 1.2 (Phase 7.3.3)
**日付**: 2026-01-15
**目的**: Spresense側統計情報をPCへ送信

### 背景

Phase 4.1でSpresense側の統計情報をリアルタイムでPC側に送信する機能を追加しました。
Phase 7でTCP統計、Phase 7.3.3でフレームドロップ統計を追加し、現在50バイト固定長です。

### Metricsパケットフォーマット

```
┌─────────────────────────────────────────────────────────────┐
│                  Metrics Packet (50 bytes)                  │
├──────────────┬──────────┬──────────┬──────────┬────────────┤
│  SYNC_WORD   │ SEQUENCE │TIMESTAMP │  DATA    │  CHECKSUM  │
│  (4 bytes)   │ (4 bytes)│ (4 bytes)│(36 bytes)│  (2 bytes) │
└──────────────┴──────────┴──────────┴──────────┴────────────┘
   0xCAFE        uint32     uint32    9×uint32     uint16
   BEEF                               (詳細下記)   CRC-16
```

**総サイズ**: 50 bytes (固定長)

**サイズ計算式**: `4 (sync) + 4*11 (fields) + 2 (crc) = 50 bytes`

### フィールド詳細

| オフセット | フィールド | サイズ | 型 | 説明 |
|-----------|-----------|--------|-----|------|
| 0x00 | sync_word | 4 | uint32 | `0xCAFEBEEF` (固定) |
| 0x04 | sequence | 4 | uint32 | Metricsパケット連番 |
| 0x08 | timestamp_ms | 4 | uint32 | Spresense起動からの経過時間 (ms) |
| 0x0C | camera_frames | 4 | uint32 | 総カメラフレーム数 |
| 0x10 | usb_packets | 4 | uint32 | 総USB/TCPパケット送信数 |
| 0x14 | action_q_depth | 4 | uint32 | アクションキュー深度 (0-7) |
| 0x18 | avg_packet_size | 4 | uint32 | 平均MJPEGパケットサイズ (bytes) |
| 0x1C | errors | 4 | uint32 | 総エラー数 |
| 0x20 | tcp_avg_send_us | 4 | uint32 | TCP送信平均時間 (μs) [Phase 7] |
| 0x24 | tcp_max_send_us | 4 | uint32 | TCP送信最大時間 (μs) [Phase 7] |
| 0x28 | dropped_frames | 4 | uint32 | ドロップフレーム数 [Phase 7.3.3] |
| 0x2C | drop_events | 4 | uint32 | ドロップイベント数 [Phase 7.3.3] |
| 0x30 | crc16 | 2 | uint16 | CRC-16-CCITT |

### Sync Word

```c
#define METRICS_SYNC_WORD  0xCAFEBEEF  // Metricsパケット識別子
```

- **値**: `0xCAFEBEEF` (固定)
- **エンディアン**: Little Endian
- **バイト配列**: `[0xEF, 0xBE, 0xFE, 0xCA]`

### CRC計算範囲

- **対象**: オフセット 0x00 ～ 0x2F (48バイト)
- **アルゴリズム**: CRC-16-CCITT (Polynomial 0x1021, Init 0xFFFF)

### 送信タイミング

- **送信間隔**: 1秒ごと（1 Hz）
- **トリガー**: Camera threadで1秒経過時に生成
- **優先度**: MJPEGフレームより低優先（キュー空きがある場合のみ送信）
...
Offset  | Field           | Value (hex)        | Value (dec) | Description
--------|-----------------|--------------------|--------------|-----------------
0x00    | sync_word       | EF BE FE CA        |              | 0xCAFEBEEF
0x04    | sequence        | 05 00 00 00        | 5            | 5番目のMetrics
0x08    | timestamp_ms    | 10 27 00 00        | 10000        | 10秒経過
0x0C    | camera_frames   | 2C 01 00 00        | 300          | 300フレーム
0x10    | usb_packets     | 96 00 00 00        | 150          | 150パケット
0x14    | action_q_depth  | 03 00 00 00        | 3            | キュー深度3
0x18    | avg_packet_size | 00 B8 00 00        | 47104        | 平均46KB
0x1C    | errors          | 00 00 00 00        | 0            | エラーなし
0x20    | tcp_avg_send_us | A0 86 01 00        | 100000       | 平均100ms
0x24    | tcp_max_send_us | 40 0D 03 00        | 200000       | 最大200ms
0x28    | dropped_frames  | 1E 00 00 00        | 30           | 30フレーム
0x2C    | drop_events     | 0A 00 00 00        | 10           | 10イベント
0x30    | crc16           | XX XX              |              | CRC-16
--------|-----------------|--------------------|--------------|-----------------
Total: 50 bytes
```
...
typedef struct metrics_packet_s
{
  ...
  uint32_t sequence;         /* Metrics sequence number */
  uint32_t timestamp_ms;     /* Spresense uptime (ms) */
  uint32_t camera_frames;    /* Total camera frames */
  uint32_t usb_packets;      /* Total USB/TCP packets sent */
  uint32_t action_q_depth;   /* Action queue depth (0-7) */
  uint32_t avg_packet_size;  /* Average MJPEG packet size */
  uint32_t errors;           /* Total error count */
  uint32_t tcp_avg_send_us;  /* Average TCP send time (μs) */
  uint32_t tcp_max_send_us;  /* Maximum TCP send time (μs) */
  uint32_t dropped_frames;   /* Total dropped frames */
  uint32_t drop_events;      /* Number of drop events */
  uint16_t crc16;            /* CRC-16-CCITT */
} __attribute__((packed)) metrics_packet_t;

#define METRICS_PACKET_SIZE  50  /* Total packet size */
```
...
pub const METRICS_PACKET_SIZE: usize = 50;
...
- **サイズ**: 50 bytes/秒
- **MJPEGトラフィック**: ~1.5 MB/秒 (30fps × 50KB)
- **オーバーヘッド**: 50 / 1,500,000 = **0.003%** (無視できるレベル)

### バージョン履歴

| バージョン | Phase | 変更内容 | サイズ |
|-----------|-------|---------|--------|
| 1.0 | 4.1 | 初期実装（基本統計） | 34 bytes |
| 1.1 | 7 | TCP統計追加 | 42 bytes |
| 1.2 | 7.3.3 | フレームドロップ統計追加 | 50 bytes |
